...
par(mfcol = c(2,5))
plot.phylo(yule.trees[[1]], cex = 0.001)
plot.phylo(yule.trees[[2]], cex = 0.001)
plot.phylo(yule.trees[[3]], cex = 0.001)
plot.phylo(yule.trees[[4]], cex = 0.001)
plot.phylo(yule.trees[[5]], cex = 0.001)
plot.phylo(yule.trees[[6]], cex = 0.001)
plot.phylo(yule.trees[[7]], cex = 0.001)
plot.phylo(yule.trees[[8]], cex = 0.001)
plot.phylo(yule.trees[[9]], cex = 0.001)
plot.phylo(yule.trees[[10]], cex = 0.001)
```
...
```{r, eval=TRUE}
my.trees1 <- TreeSim::sim.bd.taxa(n=300, numbsim=5, lambda=10, mu=0.9, complete=FALSE)

my.trees2 <- TreeSim::sim.bd.taxa(n=100, numbsim=5, lambda=10, mu=0.9, complete=FALSE)
par(mfcol = c(1,2))
...
This exercise is partially based on the [HiSSE vignette](https://cran.r-project.org/web/packages/hisse/vignettes/hisse-vignette.html) by Jeremy Beaulieu. HiSSE is not the only method in this space; I focus on it here as an example approach.

You will also need to download these files:

* [testrecon1.rda](testrecon1.rda)
* [testrecon2.rda](testrecon2.rda)
* [testrecon3.rda](testrecon3.rda)
...
viola.tree<-extract.clade(GBMB_tree, viola_node)

plot.phylo(viola.tree, cex = 0.2)
ape::ltt.plot(viola.tree)
```

```{r}
viola.trait <- sample(1:4, 173, replace = TRUE)
species.list <- viola.tree$tip.label
viola.tree$tip.state <- viola.trait
viola.tree

sim.dat.true <- data.frame(species.list, viola.tree$tip.state)
sim.dat <- sim.dat.true
# Now to hide the "hidden" state
sim.dat[sim.dat[,2]==3,2] = 1
sim.dat[sim.dat[,2]==4,2] = 2
...
viola.mod = hisse(viola.tree, sim.dat, f=c(1,1), hidden.states=TRUE, turnover.anc=turnover.anc,
           eps.anc=eps.anc, trans.rate=trans.rates.nodual.allequal)
viola.mod
```
