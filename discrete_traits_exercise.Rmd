...
viola_trait_012 <- rbinom(n=173, size=2, prob=0.3)
viola_data <- data.frame(species_list, viola_trait_012, row.names = 1)
...
```{r eval = FALSE}
viola_binary_trait <- rbinom(n=173, size=2, prob=0.5)
viola_data <- matrix(viola_binary_trait, nrow = length(species_list), ncol = 1, dimnames = list(species_list))

cleaned.discrete.phyDat <- phyDat(data = viola_data, type="USER", levels = c("0","1", "2")) #phyDat is a data format used by phangorn

anc.p <- phangorn::ancestral.pars(viola_tree, cleaned.discrete.phyDat)
...
mm = rbind(c(NA, 1), c(2, NA))

cor.rates <- rayDISC(phy = viola_tree, data = viola_data2, ntraits = 1, node.states = "marginal", rate.mat = mm)
cor.rates

```

####How could you examine if transition rates are equal?
The "fit" output from rayDISC tells you the transition rates between states. Because I supplied a matrix indicating that I think the rates would be different, it provided two different parameter estimates for the transition from 0 to 1 and 1 to 0. 
...
Because the MKV model uses discrete traits to estimate branch lengths using likelihood, using only variable characters (without correcting for acquisition bias) leads to overestimation of rates of evolution and therefore less accurate estimates of branch lengths. 

```{r}
#simulating more traits, because it needs more than one:
species_list <- viola_tree$tip.label
viola_binary_trait2 <- rbinom(n=173, size=1, prob=0.2)
viola_binary_trait3 <- rbinom(n=173, size=1, prob=0.7)
viola_binary_trait4 <- rbinom(n=173, size=1, prob=0.4)


viola_data3 <- data.frame(viola_binary_trait2, viola_binary_trait3, viola_binary_trait4, row.names = species_list)
names(viola_data3)

# MKV <- lewisMkv(phy = viola_tree, data = viola_data3, ncores = 2)
# 
# par(mfcol = (c(1,2)))
# plot.phylo(viola_tree, cex = 0.2)
# plot.phylo(MKV$phy, cex = 0.2)
...
```
I got MKV to work, but for some reason it is not working in Rmarkdown. The tree I got from it has much different branch lengths than the tree I provided it (much shorter branches).
